...
function renderStage(s){
  const icon=s.state==='complete'?'âœ“':s.state==='working'?'â†»':'â€¢';
  const finding=s.finding?`<p class="finding">${escapeHtml(s.finding)}</p>`:'';
  const sp=Number(s.percent??((s.fraction||0)*100));
  const subs=Object.entries(s.subchecklist||{}).map(([k,v])=>`<div class="subcheck-row"><span>${escapeHtml(prettyKey(k))}</span><strong>${Number(v).toFixed(2)}%</strong></div>`).join('');
  const subbox=subs?`<div class="subchecklist">${subs}</div>`:'';
  return `<article class="stage ${escapeHtml(s.state)}"><div class="stage-icon">${icon}</div><div><h3>${escapeHtml(s.name)}</h3><div class="stage-progress stage-percent"><div><i style="width:${Math.max(0,Math.min(100,sp))}%"></i></div><strong>${sp.toFixed(2)}%</strong></div><p>${escapeHtml(s.detail)}</p>${finding}${subbox}</div><div class="state">${escapeHtml(s.state)}</div></article>`;
}
function render(d){
  ...
  setText('combinedWr',pct(m.combined_win_rate));setText('combinedLabel',m.combined_label||'--');metricClass($('combinedWr'),m.combined_win_rate);  setText('tradesDay',Number(m.qualifying_trades_per_day||0).toFixed(2));setText('tradeTarget',`Target: â‰¥${m.trade_target_per_day||100}/day at â‰¥${m.target_win_rate||98}% WR`);
  const q=d.structure||{};setText('structureStage',String(q.structure_stage||'pending').replaceAll('_',' '));setText('structureProgress',`${Number(q.structure_progress||0).toFixed(2)}%`);setText('structureDays',`${q.structure_days||0}/${q.structure_days_total||183}`);setText('structureTicks',num(q.structure_ticks));setText('structureEvents',num(q.structure_events));setText('structureSuccess',`${num(q.structure_success_rows)} / ${num(q.structure_success_target)}`);setText('structureCoverage',`${Number(q.structure_coverage||0).toFixed(2)}%`);setText('structureHealth',q.structure_health||'WAITING');setText('structureError',`${q.structure_health||'WAITING'} / ${num(q.structure_error_bytes||0)} B`);
  const sb=d.structural_broker||{};setText('structBrokerStage',String(sb.stage||'pending').replaceAll('_',' '));setText('structBrokerProgress',`${Number(sb.progress||0).toFixed(2)}%`);setText('structBrokerCandidates',num(sb.raw_dollar95_candidates));setText('structBrokerGeometries',num(sb.primary_geometries||36));setText('structBrokerRaw95',num(sb.raw_broker95_combos));setText('structBrokerExact95',num(sb.exact95_structural_valid));setText('structBrokerPre98',num(sb.exact98_pretiming));setText('structBrokerHealth',sb.health||'WAITING');setText('structBrokerError',`${sb.health||'WAITING'} / ${num(sb.error_bytes||0)} B`);
  ...
  $('stageList').innerHTML=stages.map(renderStage).join('');
